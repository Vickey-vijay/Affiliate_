...
class NotificationPublisher:
    ...
    def telegram_push(self, message: str, image_path: str | None = None):
        """
        Sends a notification to multiple Telegram channels/groups.
        :param message: The message to send.
        :param image_path: Path to the image to send (optional).
        :return: Tuple (success: bool, error_message: str or None)
        """
        with open("config.json", "r") as f:
            config = json.load(f)
        bot_token = config.get("telegram_bot_token")
        channel_names = config.get("telegram_chat_id", "")

        if not bot_token or not channel_names:
            error_message = "Bot token or channel names not configured."
            print(f"[Telegram] {error_message}")
            return False, error_message

        channel_names = channel_names if isinstance(channel_names, str) else ""
        ...
        for channel_name in channel_names:
            ...
            if image_path is not None:
                if isinstance(image_path, (int, float)):
                    print(f"[Telegram] Warning: image_path was a number ({image_path}), converting to string")
                    image_path = str(image_path)
                
                # Also check if the path actually exists
                if not os.path.exists(image_path):
                    print(f"[Telegram] Warning: image file not found: {image_path}")
                    image_path = None

            if image_path:
                url = f"https://api.telegram.org/bot{bot_token}/sendPhoto"
                payload = {"chat_id": channel_name, "caption": message, "parse_mode": "HTML"}
                files = {"photo": open(image_path, "rb")}
            else:
                url = f"https://api.telegram.org/bot{bot_token}/sendMessage"
                payload = {"chat_id": channel_name, "text": message, "parse_mode": "HTML"}
                files = None

            try:
                response = requests.post(url, data=payload, files=files)
                if response.status_code == 200:
                    print(f"[Telegram] Message sent successfully to {channel_name}.")
                else:
                    error_message = response.json().get("description", "Unknown error")
                    print(f"[Telegram] Failed to send message to {channel_name}: {error_message}")
                    errors.append(f"{channel_name}: {error_message}")
            except Exception as e:
                print(f"[Telegram] Exception occurred for {channel_name}: {e}")
    ...
    def whatsapp_push(self, product, recipient_name, message=None, is_channel=False):
        ...
        try:
            ...
            success, status_msg = self.whatsapp_sender.send_message(recipient_name, message, image_path, is_channel)
